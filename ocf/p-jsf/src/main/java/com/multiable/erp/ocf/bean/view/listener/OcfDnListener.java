...
public class OcfDnListener extends MacModuleRecordViewListener implements ITradingCalcDecorator {
		...
		if (isIn(command, "printCard")) {
			...
			if (cardSize.equals("--")) {
				...
				if (cardSize.equals("Small")) {
					providerCode = "com.multiable.erp.ocf.ireport.provider.cardsizesmall.CardSizeSmallProvider";

					// Get default providerCode and docReportCode
					SqlTable defPs = dnEJB.getDefaultPrintSetting(providerCode);
					if (defPs != null && defPs.size() > 0) {
						docReportCode = defPs.getString(1, "code");
					} else {
					...
					providerCode = "com.multiable.erp.ocf.ireport.provider.cardsizelarge.CardSizeLargeProvider";
	...
	public void after_mainfooter_up(IMacCalculateHelper calcHelper, int rowIndex, SqlTable srcTable, boolean fromUc,
		...
		// int incRow = MacSqlTableLib.seek(srcTable, new String[] { getCalcHelper().getFooterProField(),
		// getCalcHelper().getFooterUnitField(), getCalcHelper().getInChargeQtyField() }, getMainFooter(),
		// rowIndex);
		//
		// if (incRow == -1) {
		// incRow = MacSqlTableLib.seek(srcTable,
		// new String[] { getCalcHelper().getFooterProField(), getCalcHelper().getFooterUnitField() },
		// getMainFooter(), rowIndex);
		// }

		Long proId = dnt.getLong(rowIndex, "proId");
		...
		if (proTable != null && proTable.size() > 0) {
			...
			Map<String, Double> reCalResult = MacVatUtil.triggerVatFormula(getBeId(), getFTName(), getMainFooter(),
					rowIndex, "disc", true, decimapMapping);
			...
			if (MacUtil.isIn(lastLookupType, "tranId")) {
				if (tarIndex == 1) {
					...
					if (proTable != null && proTable.size() > 0) {
						...
						// Cannot handle disc here coz CalcDelegateImpl will apply disc
						// Handle add on item
						// if (proTable.getBoolean(1, "ocfAddOnItem")) {
